/* ad-script.js
   ...
   Namespace: iiuo-
   Plain & Professional
   GA4 Integrated + Console Logging + Batching + Daily Cap
...
(function iiuoAds() {
  ...
  const STORAGE_KEY = "iiuo_ad_metrics_v4";
  ...
  function sendBatchedEvents(){
    if(batchedEvents.clicks.length>0){
      if(typeof gtag==="function") gtag("event","ads-iiuo-clicks",{ads:batchedEvents.clicks});
      if(DEBUG) console.log("Batched Clicks Sent:",batchedEvents.clicks);
      batchedEvents.clicks=[];
    }
    if(batchedEvents.impressions.length>0){
      if(typeof gtag==="function") gtag("event","ads-iiuo-impressions",{ads:batchedEvents.impressions});
      if(DEBUG) console.log("Batched Impressions Sent:",batchedEvents.impressions);
      batchedEvents.impressions=[];
    }
    if(batchedEvents.sponsors.size>0){
      if(typeof gtag==="function") gtag("event","ads-iiuo-sponsors",{sponsors:Array.from(batchedEvents.sponsors)});
      if(DEBUG) console.log("Batched Sponsors Sent:",Array.from(batchedEvents.sponsors));
